...
# Este script se utiliza con ejercicio10client
...
def mueveRobot(req):
    ...
    if req.tipo == 0: # Movimiento lineal
        ...
        msg = Twist()
        while terminado!=1:
            msg.linear.x = 0.13-(0.08/(req.longitud/2))*kp*abs((abs(sqrt((posicionx-posicion_inicialx)**2 + (posiciony-posicion_inicialy)**2))-req.longitud/2)) # m/s, hago que vaya más lento al principio y final            
            if obstaculo_actual < req.longitud+0.2: # Dejo un márgen de seguridad
                ...
                return InitiateMove3Response(0)
            ...
            if abs(sqrt((posicionx-posicion_inicialx)**2 + (posiciony-posicion_inicialy)**2)) > req.longitud: # Veo si he llegado al destino              
                ...
                return InitiateMove3Response(1)
            msg.angular.z = kp * (yaw_inicial - yaw) # Por si patina para que vuelva al ángulo inicial...
            pub.publish(msg)
...
if __name__ == '__main__':
    ...
    s = rospy.Service('move', InitiateMove3, mueveRobot)
